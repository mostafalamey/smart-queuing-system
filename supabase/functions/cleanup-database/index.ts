...
serve(async (req) => {
  ...
  try {
    ...
    const expectedAdminKey = Deno.env.get('CLEANUP_ADMIN_KEY') || 'cleanup-admin-2025';
    ...
    if (req.method === 'POST') {
      ...
      cleanupConfig = {
        ...
        cleanupType: (url.searchParams.get('type') as 'tickets' | 'notifications' | 'both') || 'both',
    ...
    const config: Required<CleanupRequest> = {
      ...
      successfulNotificationRetentionMinutes: cleanupConfig.successfulNotificationRetentionMinutes || 60,
      failedNotificationRetentionHours: cleanupConfig.failedNotificationRetentionHours || 24,
    ...
    if (config.organizationId) {
      ...
      if (error || !org) {
        return new Response(
          JSON.stringify({ error: 'Organization not found', details: error?.message }),
    ...
    // Calculate totals
    const totalTicketsProcessed = organizationResults.reduce((sum, r) => sum + r.ticketsProcessed, 0);
    const totalNotificationsProcessed = organizationResults.reduce((sum, r) => sum + r.notificationsProcessed, 0);
    
    // Generate global recommendations
    const globalRecommendations = generateGlobalRecommendations(organizationResults);
  ...
  try {
    ...
    if (config.cleanupType === 'tickets' || config.cleanupType === 'both') {
      try {
        ...
        console.log(`Tickets cleanup: ${ticketResult.deleted} deleted, ${ticketResult.archived} archived`);
      } catch (error) {
        result.details.errors.push(`Tickets cleanup failed: ${error.message}`);
        console.error(`Tickets cleanup failed for ${organization.name}:`, error);
      }
    }

    // Cleanup notifications if requested
    if (config.cleanupType === 'notifications' || config.cleanupType === 'both') {
      try {
        ...
        result.notificationsProcessed = notificationResult.successfulDeleted + notificationResult.failedDeleted;
        result.details.successfulNotificationsDeleted = notificationResult.successfulDeleted;
        result.details.failedNotificationsDeleted = notificationResult.failedDeleted;
        
        if (notificationResult.error) {
          result.details.errors.push(`Notifications: ${notificationResult.error}`);
        }
        
        console.log(`Notifications cleanup: ${notificationResult.successfulDeleted} successful, ${notificationResult.failedDeleted} failed deleted`);
      } catch (error) {
        result.details.errors.push(`Notifications cleanup failed: ${error.message}`);
        console.error(`Notifications cleanup failed for ${organization.name}:`, error);
      }
    }
    ...
    result.totalExecutionTimeMs = Date.now() - orgStartTime;
    console.error(`Organization cleanup failed for ${organization.name}:`, error);
  }
...
async function cleanupTicketsForOrganization(
  ...
  try {
    const { data, error } = await supabase.rpc('cleanup_old_tickets_with_notifications', {
    ...
    if (error) {
      console.error('RPC cleanup_old_tickets_with_notifications failed, falling back to direct queries:', error);
    ...
    console.error('Failed to use RPC function, falling back to direct cleanup:', error);
    return await cleanupTicketsDirect(supabase, organizationId, config);
  }
}
...
async function cleanupTicketsDirect(
  ...
  const cutoffTime = new Date(Date.now() - (config.ticketRetentionHours * 60 * 60 * 1000)).toISOString();
  ...
  try {
    ...
    if (deptError) {
      return { deleted: 0, archived: 0, error: `Failed to get departments: ${deptError.message}` };
    }
    ...
    if (fetchError) {
      return { deleted: 0, archived: 0, error: `Failed to fetch tickets: ${fetchError.message}` };
    }
    ...
    if (config.archiveTickets && !config.dryRun) {
      ...
      if (archiveError) {
        console.warn('Archive failed, but continuing with deletion:', archiveError);
      } else {
    ...
    if (!config.dryRun) {
      const { error: deleteError } = await supabase
        .from('tickets')
        .delete()
        .in('id', oldTickets.map(t => t.id));

      if (deleteError) {
        return { deleted: 0, archived, error: `Failed to delete tickets: ${deleteError.message}` };
      }
...
async function cleanupNotificationsForOrganization(
  ...
  organizationId: string,
  config: Required<CleanupRequest>
): Promise<{ successfulDeleted: number; failedDeleted: number; error?: string }> {
  ...
  try {
    ...
    if (!config.dryRun) {
      ...
      if (successError) {
        console.warn('Failed to delete successful notifications:', successError);
      }
    }
...
function generateOrganizationRecommendations(result: CleanupResult): string[] {
  ...
  if (result.details.errors.length > 0) {
    recommendations.push('⚠️ Some cleanup operations failed - check logs and retry');
  }
  
  if (result.ticketsProcessed > 1000) {
    recommendations.push('📊 High ticket volume detected - consider more frequent cleanup');
  }
  
  if (result.notificationsProcessed > 5000) {
    recommendations.push('🔔 High notification volume - consider shorter retention periods');
  }
  
  if (result.ticketsProcessed === 0 && result.notificationsProcessed === 0) {
    recommendations.push('✅ Database is clean - no cleanup needed');
  } else {
    recommendations.push(`✅ Cleaned ${result.ticketsProcessed} tickets and ${result.notificationsProcessed} notifications`);
  }
...
function generateGlobalRecommendations(results: CleanupResult[]): string[] {
  ...
  const totalErrors = results.reduce((sum, r) => sum + r.details.errors.length, 0);
  const totalTickets = results.reduce((sum, r) => sum + r.ticketsProcessed, 0);
  const totalNotifications = results.reduce((sum, r) => sum + r.notificationsProcessed, 0);
  
  if (totalErrors > 0) {
    recommendations.push(`⚠️ ${totalErrors} errors occurred during cleanup - review organization details`);
  }
  
  if (totalTickets > 10000) {
    recommendations.push('📈 Very high ticket volume across organizations - consider upgrading database plan');
  }
  
  if (totalNotifications > 50000) {
    recommendations.push('🔔 Very high notification volume - recommend aggressive cleanup schedule');
  }
  
  recommendations.push(`✅ Global cleanup complete: ${totalTickets} tickets, ${totalNotifications} notifications processed`);
