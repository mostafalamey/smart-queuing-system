...
export const usePlanLimits = () => {
  ...
  const fetchPlanLimits = useCallback(async () => {
    ...
    try {
      ...
      setPlanLimits(data)
      logger.info('Plan limits fetched:', data)
    } catch (err: any) {
  ...
  const checkLimits = useCallback((): PlanLimitCheck => {
    ...
    const canCreateBranch = planLimits.max_branches === -1 || 
                           planLimits.current_branches < planLimits.max_branches

    const canCreateDepartment = planLimits.max_departments === -1 || 
                               planLimits.current_departments < planLimits.max_departments

    const canCreateService = planLimits.max_services === -1 || 
                            planLimits.current_services < planLimits.max_services

    const canCreateStaff = planLimits.max_staff === -1 || 
  ...
  const getLimitMessage = useCallback((type: 'branch' | 'department' | 'service' | 'staff') => {
    ...
    switch (type) {
      case 'branch':
        if (!limits.canCreateBranch) {
          ...
          return nextPlan 
            ? `You've reached your ${PLAN_NAMES[planLimits.plan]} plan's branch limit (${limit}). ${limits.upgradeMessage}`
      ...
      case 'department':
        if (!limits.canCreateDepartment) {
          ...
          return nextPlan 
            ? `You've reached your ${PLAN_NAMES[planLimits.plan]} plan's department limit (${limit}). ${limits.upgradeMessage}`
      ...
      case 'service':
        if (!limits.canCreateService) {
          ...
          return nextPlan 
            ? `You've reached your ${PLAN_NAMES[planLimits.plan]} plan's service limit (${limit}). ${limits.upgradeMessage}`
      ...
      case 'staff':
        if (!limits.canCreateStaff) {
          ...
          return nextPlan 
            ? `You've reached your ${PLAN_NAMES[planLimits.plan]} plan's staff limit (${limit}). ${limits.upgradeMessage}`
  ...
  const getUsagePercentage = useCallback((type: 'branch' | 'department' | 'service' | 'staff') => {
    ...
    switch (type) {
      ...
      case 'department':
        ...
        return (planLimits.current_departments / planLimits.max_departments) * 100
  ...
  return {
    ...
    isUnlimited: (type: 'branch' | 'department' | 'service' | 'staff') => {
      ...
      switch (type) {
        case 'branch': return planLimits.max_branches === -1
        case 'department': return planLimits.max_departments === -1
        case 'service': return planLimits.max_services === -1
        case 'staff': return planLimits.max_staff === -1
