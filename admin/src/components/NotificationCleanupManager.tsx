...
export default function NotificationCleanupManager({ 
  ...
  const fetchNotificationStats = async () => {
    try {
      ...
      const { data: recommendations, error } = await supabase
        .rpc('get_organization_cleanup_recommendations')
      
      if (error) throw error
      
      // Filter by organization if specified
      const filteredData = organizationId
        ? recommendations.filter((r: any) => r.organization_id === organizationId)
        : recommendations

      setStats(filteredData.map((item: any) => ({
    ...
    try {
      ...
      if (emergency) {
        ...
        const { data, error } = await supabase
          .rpc('emergency_cleanup_all_notifications', { 
        ...
        const result = data[0]
        showSuccess(`Emergency cleanup: ${result.total_deleted} notifications deleted from ${result.organizations_affected} organizations`)
        
      } else {
        // Standard bulk cleanup
        const { data, error } = await supabase
          .rpc('safe_bulk_cleanup_notifications', {
            organization_ids: orgIds || null,
            dry_run: cleanupSettings.dryRun,
            cleanup_successful: cleanupSettings.cleanupSuccessful,
            cleanup_failed_older_than_hours: cleanupSettings.failedRetentionHours
          })
        
        if (error) throw error
        
        const totalDeleted = data.reduce((sum: number, item: any) => sum + item.total_would_delete, 0)
        const action = cleanupSettings.dryRun ? 'would delete' : 'deleted'
        
        showSuccess(`${cleanupSettings.dryRun ? 'DRY RUN: ' : ''}${totalDeleted} notifications ${action}`)
        
        // Show detailed results
        data.forEach((result: any) => {
          if (result.total_would_delete > 0) {
            console.log(`${result.organization_name}: ${result.total_would_delete} notifications ${action}`)
          }
        })
      }
  ...
  const totalCleanableLogs = stats.reduce((sum, stat) => 
    sum + stat.cleanableSuccessful + stat.cleanableFailed, 0
  )
  
  const totalEstimatedSavings = stats.reduce((sum, stat) => sum + stat.estimatedSavingsKb, 0)
    ...
    <div className={`bg-white rounded-lg shadow ${className}`}>
      ...
      <div className="p-6 bg-gray-50 border-b border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{totalCleanableLogs}</div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">
              {stats.filter(s => s.priorityLevel === 'HIGH' || s.priorityLevel === 'MEDIUM').length}
            </div>
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex flex-wrap gap-3">
          ...
          {totalCleanableLogs > 1000 && (
            <button
              onClick={() => {
                if (confirm('⚠️ This will delete ALL notification logs. Are you sure?')) {
            ...
            <div>
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => setCleanupSettings(prev => ({
                    ...prev,
            ...
            <div>
              <label className="flex items-center">
                <input
                  ...
                  className="rounded border-gray-300"
                />
                <span className="ml-2 text-sm text-gray-700">Clean failed notifications</span>
              </label>
            </div>
      ...
      {/* Organization Stats */}
      <div className="p-6">
        <h4 className="font-medium text-gray-900 mb-4">Organization Statistics</h4>
          ...
          <div className="space-y-3">
            {stats.map((stat, index) => (
              <div key={index} className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h5 className="font-medium text-gray-900">{stat.organizationName}</h5>
                    <p className="text-sm text-gray-600 mt-1">
                      {stat.totalLogs} total logs • 
                      {stat.cleanableSuccessful + stat.cleanableFailed} cleanable • 
                      {formatFileSize(stat.estimatedSavingsKb)} potential savings
                    </p>
                  </div>
                  
                  <div className="flex items-center space-x-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getPriorityColor(stat.priorityLevel)}`}>
                      {stat.priorityLevel}
                    </span>
